...
def roi_contrast(bkg_key, roi_key, capture_window=None, data_dir_prefix=''):
# bkg_key = 'background'
# roi_key = 'roi'
# if True:
    ...
    for p in range(len(dir_pairs)):
        ...
        contrast = np.divide(np.subtract(bkg_captures, roi_captures), bkg_captures, where=bkg_captures>0, out=contrast)
        
        # Check for existence of figures directory
        path_string = os.path.join('..\\' + data_dir_prefix + 'figures\\contrast_{}to{}'.format(capture_window[0], capture_window[1]), dir_pairs[p][1][0:roi_term_len])
        ...
        # Print status
        print("Plotting contrast for {} and {}".format(dir_pairs[p][0][0:bkg_term_len], dir_pairs[p][1][0:roi_term_len]))
        ...
        for s in range(number_of_sources):
            for d in range(number_of_chips):
                ...
                plt.title("Source {}, Detector {} Contrast".format(s, d))
                plt.xlabel("Time (ns)")
                plt.ylabel("Contrast")
                ymin, ymax = plt.ylim()
                if ymin < -1:
