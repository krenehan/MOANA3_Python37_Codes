...
if len(sys.argv) != 6:
    raise Exception("5 arguments expected, but received " + str(len(sys.argv)-1))
else:
...
# Finish imports following directory change
from includes import *
import matplotlib.pyplot as plt
# from KeysightE36312A import * 
from copy import deepcopy

# To check if data is good
data_bad = True
...
while data_bad:
    ...
    integration_time = meas_per_patt * 1/clk_freq
    print("Integration time per source is " + str(round(integration_time*1000, 2)) + " ms")
    print("Total integration time per source is " + str(round(integration_time*number_of_captures*number_of_frames, 4)) + " s")
    print("Total test time is " + str(round(integration_time*number_of_frames*patt_per_frame*number_of_captures, 4)) + " s")
    ...
    for time_gate_value in time_gate_list:
        ...
        if logging:
            ...
            results_dir = '../../data/spatial_resolution_mapping_tests/data'
            if not os.path.exists(results_dir):
                os.makedirs(results_dir)
            
            # Create experiment directory
            experiment_directory = os.path.join(results_dir, dir_name)
            if not os.path.exists(experiment_directory):
                os.mkdir(experiment_directory)
            
            # Create inner directory
            experiment_directory = os.path.join(experiment_directory, conditions)
            if not os.path.exists(experiment_directory):
                os.mkdir(experiment_directory)
        ...
        try:
            ...
            top_level_data_structure = np.zeros((number_of_captures, number_of_chips, number_of_frames, patt_per_frame, number_of_bins), dtype=int)
            ...
            while True:
                ...
                if dut.check_ram_trigger():
                    ...
                    # Save
                    top_level_data_structure[c] = packet.data.copy()
            ...
            # Average across capture axis
            top_level_data_structure = np.sum(top_level_data_structure, axis=(0,2), dtype=float)
            
            # Set data_bad
            if not (np.any(top_level_data_structure.flat < 0)):
                
                # Reorganize data into bin x detector x source
                source_data = np.transpose(top_level_data_structure, axes=(2,0,1))
                
                # Check that most photons arrived before bin 60
                if (np.sum(source_data[0:60]) > np.sum(source_data[60:150])):
                
                    # Update
                    data_bad = False
                
                    # Save
                    np.save(os.path.join(experiment_directory, "data.npy"), top_level_data_structure)
