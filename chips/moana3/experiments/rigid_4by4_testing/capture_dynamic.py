...
for time_gate_value in time_gate_list:
    ...
    try:
        ...
        if dynamic_mode:
            ...
            # b_arr = dynamic_packet.create_pipe_in()
        ...
        # Read packet
        # for p in range(patt_per_frame*3):
                
        #     # Advance packet
        #     dut.acknowledge_read_trigger()
                
        #     d_packet = dut.fpga_interface.wire_out(0x31)
        #     print("Final data packet for  pattern " + str(p) + ": " + np.binary_repr(d_packet, 16))
