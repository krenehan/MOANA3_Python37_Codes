...
for time_gate_value in time_gate_list:
    ...
    if logging:
        ...
        results_dir = '../../data/DNL_testing/data/'
        ...
        s = \
            ...
            "Base Subtractor Value: " + str(0) + "\n" \
            "Subtractor Value: " + str(0) + "\n" \
            "Logging: " + str(logging) + "\n" + \
            "Logging Directory: " + str(0) + "\n"
    ...
    try:
        ...
        # List for averaging
        # =============================================================================
        avg = np.empty( (number_of_captures, number_of_chips), dtype=float)
        avg.fill(np.nan)
        ...
        # Print
        print("State before blitz is " + dut.FrameController.check_state())
        ...
        while True:
            ...
            if dut.check_ram_trigger():
                ...
                # Calculate sum
                d = np.transpose(packet.data, axes=(3, 0, 1, 2))
                d[0].fill(0)
                d = np.transpose(d, axes=(1, 2, 3, 0))
                photons = np.sum(d, axis=(1,2,3))
                print("Chip 0 photons: " + str(photons[0]))
                
                # Fill array
                avg[c] = photons
                
                # Print averages
                rt_avg = np.nanmean(avg, axis=(0))
                for chip in range(number_of_chips):
                    print("Average of " + str(c) + " captures for chip " + str(chip) + ": " + str(round(rt_avg[chip], 0)) + " photons")
